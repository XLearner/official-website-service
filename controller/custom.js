...
async function Add(ctx) {
  const { title, logo, description, ifShow } = ctx.request.body;

  if (!title || !logo || !description || !ifShow) {
    ctx.body = utils.jsonback(-1, "", "参数不全");
    return;
  }

  const keys = ["title", "logo", "description", "ifShow"];
  const values = [title, logo, description, ifShow]
    .map((ele) => `"${ele}"`)
    .join(",");
